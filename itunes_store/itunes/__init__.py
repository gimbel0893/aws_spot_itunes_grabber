...
class Search(_BaseObject):
    ...
    def __init__(self, query, country=COUNTRY, media='all', entity=None, attribute=None, limit=50, lang='en_us', version=API_VERSION, explicit='Yes', offset=0):
        ...
        self._search_terms['offset'] = offset     # for pagination so we can access search results past the first 200
...
class Software(Track):
    ...
    def _set(self, json):
        ...
        self._set_title(json)
        self._set_developer(json)
        self._set_publisher(json)
    ...
    def _set_genres(self, json):
        # remove main genre from list
        self.genres = [genre for genre in json.get('genres', None) \
    ...
    def _set_title(self, json):
        self.title = json.get('trackName', None)

    def _set_developer(self, json):
        self.developer = json.get('artistName', None)

    def _set_publisher(self, json):
        self.publisher = json.get('sellerName', None)

    def _set_screenshots(self, json):
        self.screenshots = json.get('screenshotUrls', [])
        self.screenshots += json.get('ipadScreenshotUrls', [])
    ...
    def get_icon(self):
        return self.get_artwork()['60']
    def get_medium_icon(self):
        return self.get_artwork()['512']
    def get_large_icon(self):
        return self.get_artwork()['100']
...
def search(query, media='all', limit=100, store=COUNTRY, offset=0):
    return Search(query=query, media=media, limit=limit, country=store, offset=offset).get()
